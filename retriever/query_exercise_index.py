...
def query_index(query: str, index_path: Path, *, top_k: int = 10, pool_size: int = 50, backend: str = "auto") -> List[Dict[str, Any]]:
    ...
    candidates = None if backend == "tfidf" else _dense_candidates(query, index_path, item_by_id, pool_size=pool_size)
    if backend == "dense" and candidates is None:
        raise RuntimeError("Dense Chroma index is unavailable. Rebuild with --backend dense or use --backend tfidf.")
    if candidates is None:
        candidates = _tfidf_candidates(query_features, index_path, items, pool_size=pool_size)
